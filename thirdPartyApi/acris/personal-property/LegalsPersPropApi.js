...
class LegalsPersPropApi {
    ...
    static async fetchAcrisRecords(legalsQueryParams) {
        try {
            const url = SoqlUrl.constructUrl(legalsQueryParams, "LegalsPersPropApi", "records");
            const headers = {
    ...
    static async fetchAcrisRecordCount(legalsQueryParams) {
        try {
            const url = SoqlUrl.constructUrl(legalsQueryParams, "LegalsPersPropApi", "countAll");
            const headers = {
                "Content-Type": "application/json",
                "X-App-Token": process.env.APP_TOKEN,
            };
     ...
     */
    static async fetchAcrisDocumentIdsCrossRef(legalsQueryParams, partiesRecordsDocumentIds, batchSize = 500) {
        try {
            ...
            if (allDocumentIds.size === 0) {
                throw new NotFoundError("No Personal Property Legals records found for the given query.");
            }
