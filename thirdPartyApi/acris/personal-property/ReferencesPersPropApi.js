...
class ReferencesPersPropApi {
    ...
    try {
      ...
      if (!data?.length) {
        throw new NotFoundError(
          "No records found for the given query from Personal Property References API."
        );
      }
      return data;
    } catch (err) {
      throw new Error(
        "Failed to fetch records from Personal Property References API"
      );
    }
  }
  ...
  static async fetchAcrisRecordCount(referencesQueryParams) {
    try {
      ...
      if (!data?.length || !data[0]?.count) {
        throw new NotFoundError(
          "No count data found for the given query from Personal Property References API."
        );
      }
      return Number(data[0].count);
    } catch (err) {
      throw new Error(
        "Failed to fetch record count from Personal Property References API"
      );
    }
  }
  ...
  static async fetchAcrisDocumentIds(referencesQueryParams) {
    try {
      ...
      if (!data?.length) {
        throw new NotFoundError(
          "No document IDs found for the given query from Personal Property References API."
        );
      }
      return data.map((record) => record.document_id);
    } catch (err) {
      throw new Error(
        "Failed to fetch document IDs from Personal Property References API"
      );
    }
  }
  ...
  static async fetchAcrisDocumentIdsCrossRef(
    ...
    try {
      ...
      if (allDocumentIds.size === 0) {
        throw new NotFoundError(
          "No Personal Property References records found for the given query."
        );
      }
