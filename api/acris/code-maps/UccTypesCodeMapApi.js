...
/** Functions for interacting with the ACRIS UCC Collateral Codes API. */

class UccTypesCodeMapApi {
    /**  These codes are already seeded in the database which the server will use to query the Real Property and Personal Property datasets.  This file will be used to periodically check that the UCC Collateral codes in the database match the current codes provided by the ACRIS API.  If the codes do not match, the server will update the database with the new codes.
     *
    ...
    static async fetchFromAcris(query) {
        const url = `${API_ENDPOINTS.uccCollateralCodes}?${new URLSearchParams(query).toString()}`;
...
module.exports = UccTypesCodeMapApi;