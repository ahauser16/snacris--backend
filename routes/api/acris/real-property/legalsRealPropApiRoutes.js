...
router.get("/fetchRecord", async function (req, res, next) {
  try {
    ...
    const realPropLegalsRecords = await LegalsRealPropApi.fetchAcrisRecords(
      query
    );
...
router.get("/fetchRecordCount", async function (req, res, next) {
  try {
    const query = req.query;
    const realPropLegalsRecordCount =
      await LegalsRealPropApi.fetchAcrisRecordCount(query);
...
router.get("/fetchDocIds", async function (req, res, next) {
  try {
    const query = req.query;
    const realPropLegalsRecordsDocumentIds =
      await LegalsRealPropApi.fetchAcrisDocumentIds(query); // <-- old code
...
router.get("/fetchDocIdsCrossRefPartyDocIds", async function (req, res, next) {
  try {
    ...
    const legalsDocIdsCrossRefParties =
      await LegalsRealPropApi.fetchDocIdsFromAcrisCrossRefParties(
        query,
...
router.get("/fetchAcrisRecordsByDocumentIds", async function (req, res, next) {
  try {
    ...
    const realPropLegalsRecords =
      await LegalsRealPropApi.fetchAcrisRecordsByDocumentIds(documentIds);
    return res.json({ realPropLegalsRecords });
  } catch (err) {
    return next(err);
  }
