 ...
 *
 * Technical Explanation: This route fetches data from the ACRIS-Personal Property Master API based on the query parameters provided. It constructs the URL using URLSearchParams and makes a GET request to the API. It returns the fetched records. This route was used during development for Postman-based API exploration and is deployed in the production application.
 *
 * Use Case: This route is used to fetch data from the ACRIS-Personal Property Master API based on user-provided query parameters. The server can then cross-reference this data with other datasets.
 **/
...
router.get("/fetchRecordCount", async function (req, res, next) {
  // GET /fetchRecordCount => { masterRecordCount: number }
  // Fetch the count of matching records from the ACRIS-Personal Property Master dataset.
  // Authorization required: logged-in user/admin
  try {
    const query = req.query;
    const masterRecordCount = await MasterPersPropApi.fetchCountFromAcris(
      query
    );
...
router.get("/fetchDocIds", async function (req, res, next) {
  // GET /fetchDocIds => { masterRecordsDocumentIds: [...] }
  // Fetch document IDs from the ACRIS-Personal Property Master dataset.
  // Authorization required: logged-in user/admin
  try {
    const query = req.query;
    const masterRecordsDocumentIds =
      await MasterPersPropApi.fetchDocIdsFromAcris(query);
