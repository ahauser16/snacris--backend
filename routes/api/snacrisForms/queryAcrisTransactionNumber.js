...
router.get("/fetchRecord", async function (req, res, next) {
  try {
    ...
    if (masterSearchTerms?.transaction_number) {
      masterQueryParams.transaction_number =
        masterSearchTerms.transaction_number;
      transactionNumberToQuery = masterSearchTerms.transaction_number;
    } else {
      return res
        .status(400)
        .json({ error: "Must provide a transaction number." });
    }
    ...
    try {
      ...
      const partiesRecords =
        partiesResult.status === "fulfilled" ? partiesResult.value : [];
      const legalsRecords =
        legalsResult.status === "fulfilled" ? legalsResult.value : [];
      const referencesRecords =
        referencesResult.status === "fulfilled" ? referencesResult.value : [];
      const remarksRecords =
        remarksResult.status === "fulfilled" ? remarksResult.value : [];
