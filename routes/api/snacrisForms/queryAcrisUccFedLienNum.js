...
router.get("/fetchRecord", async function (req, res, next) {
    try {
        console.log("'queryAcrisUccFedLienNum' received request with query parameters:", req.query);
        ...
        if (masterSearchTerms?.ucc_lien_file_number) masterQueryParams.ucc_lien_file_number = masterSearchTerms.ucc_lien_file_number;

        // Validate required master params
        if (!masterQueryParams.ucc_lien_file_number) {
            return res.status(400).json({ error: "ucc_lien_file_number is required in masterSearchTerms." });
        }
        ...
        try {
            ...
            const masterRecordsDocumentIds = await MasterPersPropApi.fetchAcrisDocumentIds(masterQueryParams);
            ...
            if (masterRecordsDocumentIds && masterRecordsDocumentIds.length > 0) {
                legalsRecordsDocumentIds = await LegalsPersPropApi.fetchAcrisDocumentIdsCrossRef(
                    legalsQueryParams,
        ...
        try {
            const [
                ...
                MasterPersPropApi.fetchAcrisRecordsByDocumentIds(crossReferencedDocumentIds),
                PartiesPersPropApi.fetchAcrisRecordsByDocumentIds(crossReferencedDocumentIds),
                LegalsPersPropApi.fetchAcrisRecordsByDocumentIds(crossReferencedDocumentIds),
                ReferencesPersPropApi.fetchAcrisRecordsByDocumentIds(crossReferencedDocumentIds),
                RemarksPersPropApi.fetchAcrisRecordsByDocumentIds(crossReferencedDocumentIds)
            ]);
            ...
            console.log(results, "queryAcrisUccFedLienNum");
