...
function analyzeResults(records) {
  ...
  keysToCheck.forEach((key) => {
    ...
    const mostCommonCount =
      valueCounts[sortedValues[0] === null ? "null" : sortedValues[0]] || 0;
...
router.get("/fetchRecord", async function (req, res, next) {
  try {
    ...
    if (street_number)
      queryParams.street_number = transformForUrl(street_number);
    ...
    // Attempt to fetch records; on error return status: "failed"
    let records;
    try {
      records = await LegalsRealPropApi.fetchAcrisRecords(queryParams);
    } catch (err) {
      return res.json({
        status: "failed",
        message: `Error fetching records: ${err.message}`,
      });
    }
