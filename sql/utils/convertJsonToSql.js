...
function convertRealPropertyLegalsDataToSql(jsonArray) {
  let sql = "INSERT INTO acris_real_property_legals (document_id, record_type, borough, block, lot, easement, partial_lot, air_rights, subterranean_rights, property_type, street_number, street_name, good_through_date)\nVALUES\n";

  const values = jsonArray.map(item => {
    const streetNumber = item.street_number ? `'${item.street_number}'` : 'NULL';
...
function convertRealPropertyReferencesDataToSql(jsonArray) {
  let sql = "INSERT INTO acris_real_property_references (document_id, record_type, reference_by_crfn_, reference_by_doc_id, reference_by_reel_year, reference_by_reel_borough, reference_by_reel_nbr, reference_by_reel_page, good_through_date)\nVALUES\n";

  const values = jsonArray.map(item => {
    const referenceByCrfn = item.reference_by_crfn_ ? `'${item.reference_by_crfn_}'` : 'NULL';
    const referenceByDocId = item.reference_by_doc_id ? `'${item.reference_by_doc_id}'` : 'NULL';
