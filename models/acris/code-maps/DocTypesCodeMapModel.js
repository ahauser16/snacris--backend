...
class DocTypesCodeMapModel {
   ...
   *
   * Throws BadRequestError if doc_type already exists.
   */
  static async createRecord(data) {
    ...
    if (duplicateCheck.rows[0]) {
      throw new BadRequestError(`Duplicate doc_type: ${data.doc_type}`);
    }
  ...
  static async getDocTypesByClass(doc_class) {
    ...
    if (result.rows.length === 0) {
      throw new NotFoundError(
        `No doc_type values found for class: ${doc_class}`
      );
    }
